...
export const signup=async (req,res)=>{
    try {
        ...
        if(password!==confirmPassword) return res.status(400).json({error:"Passwords doesn't match"})
...
export const logout=async(req,res)=>{
    try {
        res.cookie("jwt","",{maxAge:0})//normalde "" olan kısımda token verirdik bu token'a göre de jwt hazırlardı.
